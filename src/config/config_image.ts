...
export class ImageConfig {
    ...
    static register() {
        ...
        seal.ext.registerStringConfig(ImageConfig.ext, "图片识别默认prompt", '分析这张图片，以JSON格式输出：text1为OCR识别原文（无则空字符串），text2为50字以内图片主要内容/动作和关键重点描述，isEmoji为是否表情包（true/false）。只输出JSON。', "");
