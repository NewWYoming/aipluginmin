...
Tools using this pattern: `attr_show`, `attr_get`, `attr_set`, `jrrp`, `roll_check`, `san_check`, `modu_roll`, `modu_search`, and `run_command` (a meta-tool that dispatches to any whitelisted extension command at runtime rather than wrapping a single fixed one).
...
**`CmdArgs` runtime polyfill.** SealDice normally attaches helper methods (`getArgN`, `getKwarg`, `getRestArgsFrom`, `isArgEqual`, `chopPrefixToArgsWith`, `eatPrefixWith`) onto every `CmdArgs` it constructs. Because `extensionSolve()` builds its `cmdArgs` from scratch (a plain `{}` cast to `seal.CmdArgs`), those methods are missing and any delegated command that calls them will throw. `extensionSolve()` now re-attaches minimal polyfills at the top of every invocation:

```typescript
cmdArgs.getArgN          = (n) => cmdArgs.args[n - 1] ?? '';
cmdArgs.getKwarg         = (key) => cmdArgs.kwargs.find(k => k.name === key);
cmdArgs.getRestArgsFrom  = (n) => cmdArgs.args.slice(n - 1).join(' ');
cmdArgs.isArgEqual       = (n, ...s) => s.includes(cmdArgs.getArgN(n));
cmdArgs.chopPrefixToArgsWith = (..._s) => false;
cmdArgs.eatPrefixWith    = (..._s) => [cmdArgs.rawArgs, false];
```
...
**API shape.** 3 parameters: `extension` (string, ext name), `command` (string, command name within that ext), `args` (optional `string[]` of positional arguments). Mirrors how a user would type `.coc7 ra 力量 50` at the command line.

**Dynamic description via `getInfo` override.** Unlike other tools whose description is frozen at construction, `run_command` overrides `tool.getInfo` to call `buildDescription()` on every read. `buildDescription()` walks `ConfigManager.tool.allowedExtensions`, queries each via `seal.ext.find` for its `cmdMap`, samples up to 5 command names per ext (filtered through the blacklist), and emits a usage example. The LLM therefore always sees an up-to-date list of reachable commands, including any extensions the operator has added since startup.

**6-stage validation gate.** Before invoking `extensionSolve()`, `tool.solve` walks the inputs through these checks in order, returning a self-correction message at the first failure:

1. **Extension whitelist** — `extension` must be in `ConfigManager.tool.allowedExtensions` (trimmed, case-sensitive). Failure: list currently allowed extensions.
2. **Extension exists** — `seal.ext.find(extension)` must resolve. Failure: list currently allowed extensions.
3. **Command blacklist** — `isBlacklistHit(command, extension)` against `ConfigManager.tool.commandBlacklist`, supporting both `ext:cmd` (scoped) and bare-name (any-ext) formats. Failure: "指令禁止通用调用".
4. **Command exists** — `ext.cmdMap.hasOwnProperty(command)`. Failure: list available (non-blacklisted) commands for that ext, truncated to 200 chars.
5. **`disabledInPrivate` gate** — `ext.cmdMap[command].disabledInPrivate && ctx.isPrivate`. Failure: "不支持私聊".
6. **Execution** — `await ToolManager.extensionSolve(ctx, msg, ai, { ext, name, fixedArgs: [] }, cmdArgs, [], [])`, wrapped in try/catch.

**Self-correcting error messages.** Every validation failure is phrased so the LLM can fix its next `tool_call` without operator help: failure responses enumerate the *currently valid* options (allowed extensions, available commands for the targeted ext) rather than just stating what's wrong. This reduces the LLM's retry cost and avoids leaving the user staring at an opaque error.

**Arg normalization.** `normalizeArgs()` accepts `string` (whitespace-split), `string[]` (passed through with `String()` coercion), and `null`/missing (→ `[]`). Non-array non-string inputs return `[]` rather than erroring, since the field is optional.
