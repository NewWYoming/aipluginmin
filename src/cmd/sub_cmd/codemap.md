...
| **impression** | `impression.ts` | U (root) | View user impressions. Default: show own impression in current session (`U`). `@user`: show that user's impression (`U`). `all @user`: cross-group scan all cached AIs for that user's impressions, sorted by recency (`M`). Uses `AIManager.cache` for cross-group lookup. |
